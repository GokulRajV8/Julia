...
public class JuliaThread extends Thread {
	...
	public JuliaThread(int threadId, ComplexPlaneCanvas canvas, int xStart, int yStart, int width, int height) {
		...
		this.width = width;
		this.height = height;
		this.isJulia = !Objects.isNull(this.canvas.juliaCenter);
		this.progress = 0;
		this.isActive = false;
	}
	...
	public void run() {
		...
		if(this.isJulia) {
			...
			for(int i = 0; i < this.width; ++i) {
				...
				// updating progress
				this.progress = (100 * (i + 1)) / this.width;
			}
		}
		else {
			z.value(0.0f, 0.0f);
